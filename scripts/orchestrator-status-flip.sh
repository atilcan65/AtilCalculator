...
#   5. Wake next role: notify.sh -w -r <role> (dual-channel per ADR-0033) with -l fallback
...
if [ -n "$NEXT_ROLE" ] && [ -x "$(dirname "$0")/notify.sh" ]; then
  # Try dual-channel (-w) first, fall back to legacy (-l)
  NOTIFY_SH="$(dirname "$0")/notify.sh"
  if "$NOTIFY_SH" -w -r "$NEXT_ROLE" "$WAKE_MSG" 2>/dev/null; then
    WAKE_RESULT="dual-channel"
  elif "$NOTIFY_SH" -l "$NEXT_ROLE" "$WAKE_MSG" 2>/dev/null; then
