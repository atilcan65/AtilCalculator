...
# (scripts/wip-idle-detect.sh, Issue #291 dev impl, Sprint 6).
#
# Why this test exists
# --------------------
# ADR-0039 (Issue #289 doctrine, MERGED 1150fdb6) defines a 30-min idle
# threshold for `WIP > 0` agents. Issue #291 owns the dev implementation:
# `scripts/wip-idle-detect.sh`. d034 guards the contract:
#
...
#
# Sister test: atilcan65/dev-studio-template scripts/tests/d034-proactive-wip-idle.sh
# (template port — Issue #290 follow-up, deferred per Sprint 2+3 scope decision)
#
...
if [ "$FAIL" -gt 0 ]; then
  echo
  echo "Issue #291 REGRESSION FAILED — wip-idle-detect.sh contract incomplete."
  echo "Fix: ensure all 8 TUs pass per ADR-0039 §Decision + arch review 4778117427."
  exit 1
fi
echo
echo "Issue #291 REGRESSION PASS — wip-idle-detect.sh 8/8 contract honored."
exit 0