#!/usr/bin/env bash
# d028-no-standby.sh — regression test for Issue #238 (P0 self-standby doctrine).
#
...
#
# Fix (this PR #245, sub-tasks 2+3 of Issue #238):
...
#   3. agent-state.sh: v5 → v6 schema bump adds `last_is_alive_utc: null`
#      field with backfill for existing state files.
#
...
#   T5: state v5 → v6 backfill missing (existing state files break)
...
# ============================================================================
# T5: state v5 → v6 backfill for existing files
# ============================================================================
section "T5: state v5 → v6 backfill for last_is_alive_utc"
if grep -Eq 'v5.*v6.*backfill|has\("last_is_alive_utc"\)' "$STATE_SH"; then
  pass "v5 → v6 backfill present for last_is_alive_utc"
else
  fail "v5 → v6 backfill missing" "expected 'has(\"last_is_alive_utc\")' backfill guard in cmd_init"
fi
...
if [ "$FAIL" -gt 0 ]; then
  printf "  ${R}FAIL${D}: %d / %d\n" "$FAIL" "$TOTAL"
  echo
  echo "Issue #238 sub-task 3 REGRESSION FAILED — one or more forbidden-standby defenses missing."
  exit 1
fi
echo
echo "Issue #238 sub-task 3 REGRESSION PASS — 4 forbidden standby modes are defended:"
echo "  - 'blocked on dependency' → D7 dep_broken (ADR-0037) catches; agent takes OTHER items"
