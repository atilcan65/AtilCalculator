#!/usr/bin/env bash
# d032-rca-19-status-transition-wake.sh — regression for #269
# (RCA-19 status-transition wake fix per ADR-0036)
#
# Why this test exists
# --------------------
# Issue #231 (RCA-19): orchestrator's status flip via raw `gh issue edit`
# doesn't wake the assigned agent's watcher (8h 42min idle on Issue #222).
...
#           flips status + emits wake signal (notify.sh -w with -l fallback)
#
# d025 numbering conflict: ADR-0036 §d025 spec proposed "d025-status-transition"
# or "d027", but both are taken (d025 = cmd_set JSON contract per ADR-0034,
# d027 = state-recovery per Issue #237). Using d032 as next free number.
#
...
#   T6:  Helper has wake signal (calls notify.sh -w with -l fallback)
...
# ============================================================================
# T6: Wake signal (notify.sh -w with -l fallback)
# ============================================================================
section "T6: Helper calls notify.sh -w (with -l fallback) for wake"
if [ -f "$HELPER_SH" ] && grep -Eq 'notify\.sh.*-w|notify\.sh.*-l' "$HELPER_SH"; then
  pass "helper emits wake signal via notify.sh"
else
  fail "no wake signal" "expected: notify.sh -w -r \$NEXT_ROLE with fallback to notify.sh -l \$NEXT_ROLE (ADR-0036 §Part C property 5)"
fi
...
if [ "$FAIL" -gt 0 ]; then
  echo
  echo "Issue #269 REGRESSION FAILED — RCA-19 status-transition wake impl incomplete."
...
echo
echo "Issue #269 REGRESSION PASS — RCA-19 status-transition wake impl complete."
exit 0