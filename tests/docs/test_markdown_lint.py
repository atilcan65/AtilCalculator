...
def _discover_markdown_files() -> list[Path]:
    ...
    and agent scratch worktrees per TD-009 — `atilcalc-{role}-{task}/`).

    Scratch worktrees are per-agent working copies (per `.claude/CLAUDE.md`
    §Autonomy Loop + TD-009). They are NEVER part of the project tree
    (gitignored via `atilcalc-*/` pattern) and must not be linted as
    project content — otherwise a peer's local draft can pollute the
    developer's CI green (recurring pattern observed 2026-06-19 during
    STORY-012 impl).
    """
    md_files: list[Path] = []
    for pattern in ("*.md", "**/*.md"):
        for path in REPO_ROOT.glob(pattern):
            ...
            if any(p in (".venv", ".git", "__pycache__", "node_modules") for p in parts):
                continue
            # Exclude agent scratch worktrees (TD-009 + `.gitignore` `atilcalc-*/`).
            # A scratch dir starts with `atilcalc-` and contains a duplicate of
            # project docs (the agent's local working area).
            if any(p.startswith("atilcalc-") for p in parts):
                continue
