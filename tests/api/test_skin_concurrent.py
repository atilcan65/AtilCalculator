...
try:
    # Set up the test DB path BEFORE the app import (see test_skin_persistence.py
    # for the import-order bug rationale; main.py's import-time init_db reads
    # HISTORY_DB_PATH at module load, so the env var must be set first).
    _TEST_DB_PATH = Path(
        os.environ.get(
            "HISTORY_DB_PATH",
