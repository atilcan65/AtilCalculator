...
- **AC5** — GIVEN the browser sends a duplicate `POST /api/history` (same expr+result+ts within 60s) with same `Idempotency-Key` THEN the server returns the cached response without writing a duplicate record (idempotency per ADR-0019 §Idempotency keys).
  - **AC5.a** — *(Cache window semantics, per [ADR-0022 §Idempotency contract](docs/decisions/ADR-0022-persistence-layer.md) + [PR #82 P3 #3 review](https://github.com/atilcan65/AtilCalculator/pull/82), [Issue #86](https://github.com/atilcan65/AtilCalculator/issues/86))*: the 60s cache is **per-FastAPI-process in-memory**. Replay within 60s **in the same process** returns 200 with the cached response; replay from a **different process or device** (cross-process / cross-device) returns 201 fresh — the DB UNIQUE constraint on `idempotency_key` prevents a duplicate row. Both responses succeed per the idempotency contract; the differing status codes are acceptable. Data consistency (no duplicate rows) is the source of truth; the 60s cache is a per-process latency optimization, not a cross-device consistency mechanism. PM-tested via PR #79 TC-5 (same-process replay) + AP-12 (cross-process DB UNIQUE uniqueness).
