...
| `/var/log/dev-studio/AtilCalculator/agent-state/*.json` | Orta (regenerate edilebilir) | Günlük |
| `logs/` | Düşük | Haftalık |
...
(default: `/var/log/dev-studio/AtilCalculator/agent-state/`). Tümünü tek tarball'a al:

```bash
mkdir -p backups
tar czf backups/agent-state-$(date +%Y%m%d-%H%M).tar.gz \
    -C /var/log/dev-studio/AtilCalculator agent-state
```

Bunu cron'a koy (günlük):
```cron
0 2 * * * tar czf /home/atilcan/backups/agent-state-$(date +\%Y\%m\%d).tar.gz -C /var/log/dev-studio/AtilCalculator agent-state
```
