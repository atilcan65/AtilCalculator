...
### GH-hosted runner removal — DEFERRED to Sprint 23+ (cycle ~#1602, 2026-06-30T13:34+03:00)

**Owner decision cycle ~#1602**: "plana sadık kalcaz" — Sprint 22 PIVOT scope unchanged. GH-hosted runner (`ubuntu-latest`) REMOVAL deferred to Sprint 23+ planning alongside:
- R3 SPOF (1 VM → 2 redundancy)
- R7 runner label convention audit (Faz 1.5)
- R8 VM CPU/RAM capacity monitoring
- Faz 5.9 real-env re-test (production-grade concurrent + failover)

**Rationale**: Sprint 22 PIVOT 5-Phase Plan does NOT include "remove GH-hosted" as a step. DoD criterion #2 already satisfied by "8 self-hosted runners registered + GREEN + labelled" — no GH-hosted removal requirement. Sprint scope expansion mid-pivot = RETRO-005 #18 soul-level drift. Cold-cache self-hosted baseline data not yet established (first run in progress cycle ~#1602).

**Sprint 23+ planning entry**: Sprint 23 kickoff → bundle GH-hosted migration with R3 SPOF resolution + R7 audit + 2. VM redundancy → data-driven decision.
