...
This trigger set is **strictly label-ownership-based**. It does NOT catch `@<role>` mentions in issue/PR comments. The author (architect) discovered this gap on the same day ADR-0025 was filed: a 13-min-old `@architect` mention in [Issue #73 cmt #4748644731](https://github.com/atilproject/AtilCalculator/issues/73#issuecomment-4748644731) (from @developer, 2026-06-19T05:02:59Z) was missed by the architect agent until the human chat-level wake-up at 2026-06-19T05:16Z (per the architect's own cmt #4749006175 "Doctrinal note"). The label-based filter excluded it because Issue #73 was `agent:developer | cc:architect` (cc, not agent) — but the comment text `@architect` was the actionable signal, and the queue filter didn't see it.
