...
| **D8** | `scope_drift` | Issue's `updatedAt` is within 48h of its `status:ready` flip AND body `acceptance criteria` count has increased since `status:ready` (suggests mid-sprint scope add) | `proactive_scan` (aggregated) with `routing: product-manager` | `@product-manager` via `notify.sh -l product-manager` (PM gates the change) |
...
| 5 | Issue's `acceptance criteria` count increased since `status:ready` flip → D8 fires | happy path |
...
- D8 catches scope creep before sprint review
...
- D8 detection (acceptance criteria count) is brittle (issues may be edited for other reasons — clarification, not scope add). False-positive risk: medium. Fallback: PM judges per-alert, doesn't auto-act.
