...
| [ADR-0021](./ADR-0021-docs-pr-convention.md) | Docs PR Convention (default `agent:<author>` only; peer `cc:*` requires cross-cutting rationale) | Accepted (amended 2026-06-19) | 2026-06-18 | @architect, @orchestrator, @dev, @tester | Closes TD-006 subclass on docs PRs. Sister to ADR-0020. Accepted via PR #62. **Amended 2026-06-19** via PR (pending) to add §Status flip responsibility matrix (per ADR-0025). |
...
| [ADR-0025](./ADR-0025-bound-standby-exception.md) | Bound Standby Exception (a) + Queue-Empty Wake (Anti-Stall Guard) | Proposed | 2026-06-19 | @architect | Closes TD-006 standby-extension class. Sister to ADR-0020 + ADR-0021 + ADR-0024 (the four ADRs together close the TD-006 family). Implements Issue #109 ACs. Pending: human mirrors doctrine text into `docs/CLAUDE.md`; @developer implements `query_queue_empty` in `scripts/agent-watch.sh`; @atilcan65 approves doctrine change. |
...
- **ADR-0024** (proposed 2026-06-19) — stale-verdict watchdog schema redesign. Pending review by @product-manager (business: 1-sprint validation window), @developer (implementation: `query_stale_verdict` + `query_missing_expectation`), @tester (regression test d012), @atilcan65 (doctrine change approval). Implementation in flight: PR #108 (`feat(watchdog): ADR-0024 stale-verdict schema impl`).
- **ADR-0025** (proposed 2026-06-19) — bound standby exception + queue-empty wake. Pending review by @orchestrator (board hygiene impact), @developer (implementation: `query_queue_empty` in `scripts/agent-watch.sh`), @tester (regression test d013), @atilcan65 (doctrine change approval). Amends ADR-0021 with §Status flip responsibility matrix (same PR). Closes Issue #109.
