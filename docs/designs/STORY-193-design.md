...
| R2 | Symlink removal leaves scripts that still expect `/home/atilcan/projects/AtilCalculator` | L | **CORRECTED 2026-06-24T19:08Z (TD-030 closeout, see ADR-0045 lens (j))**: 27 refs in `scripts/.tmux-bootstrap/*.sh` are `.gitignore`d (`.gitignore` line 67) and **auto-generated** by `scripts/dev-studio-start.sh` line 27 dynamic `REPO_ROOT` resolution; canonical runner path `/home/atilcan/actions-runner/_work/AtilCalculator/AtilCalculator` **no longer exists** post-owner-ops Steps 6+7 (verified via `ls -la` at design amendment time). System self-healed via `dev-studio-start.sh` regen — post-unlink regenerated bootstrap files use dynamic `$REPO_ROOT` pointing to the new real dir, not the symlink path (verified 0 hardcoded path refs via `grep -c`). Original R2 mitigation ("no active code path uses it") was wrong on TWO counts: (a) auto-gen file status not enumerated, (b) canonical-path existence assumed not verified. See ADR-0045 §Context for full RCA + ADR-0045 §Decision for lens (j) prevention. Closes #194 SYMNK-CLEANUP via Option A (no-op verify + docs PR). |
