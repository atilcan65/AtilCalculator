...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # Parse with exit-on-usage-error swallowed: a missing expression
    # is treated as an engine error (empty input → ExpressionSyntaxError),
    # not a usage error. This matches the TC-5 contract (`atilcalc ""` →
    # stderr + non-zero exit) where the test expects exit=1 for empty
    # input, not exit=2.
    try:
        args = parser.parse_args(list(argv))
    except SystemExit as exc:
        # argparse called sys.exit(2) for usage error (unknown flag, etc.).
        # Preserve that exit code for argparse-native cases (bad flags) but
        # treat missing positional as engine error.
        if exc.code == 2 and not argv:
            return _handle_empty_expression()
        raise

    # If argparse returned with no expression (no positional args at all),
    # delegate to engine for a clean parse error.
    if not args.expression:
        return _handle_empty_expression()

    # Join all remaining args with spaces (handles both quoted and split forms)
